...
function handleApiAiAction(sender, action, responseText, contexts, parameters) {
	switch (action) {
		case "get-current-weather":
					...
					qs: {
						...
						if(!error && response.statusCode == 200){
							...
							if(weather.hasOwnProperty("weather")){
								...
								sendTextMessage(sender,'No weather for ${parameters["geo-city"]}.');
							}
						}
						else {
			...
			else if (parameters.hasOwnProperty("cardinal") && parameters.hasOwnProperty("cardinal1") && parameters["cardinal"] !='' && parameters["cardinal1"] !='') {
				var request = require('request');
				request({
					url: 'http://api.openweathermap.org/data/2.5/weather', //URL to hit
					qs: {
						appid: config.WEATHER_API_KEY,
						lon: parameters["cardinal"],
						lat: parameters["cardinal1"]
					},
				}, function(error, response, body){
						if(!error && response.statusCode == 200){
							let weather = JSON.parse(body);
							if(weather.hasOwnProperty("weather")){
								...
								sendTextMessage(sender,'No weather for ${parameters["cardinal"]} $parameters["cardinal1"].');
							}
						}
						else {
							console.log(response.error);
